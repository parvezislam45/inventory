...
export default function OrderPage() {
  ...
  const discountAmount = (() => {
    ...
    const discountPercent =
      selectedDiscount === 'discount_harvest'
  ...
  const placeOrder = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.push(`/harvest/invoices/${data.id}`);
      } else {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-3xl shadow-2xl overflow-hidden border border-gray-100">
          ...
          <div className="p-8 border-b border-gray-100">
            {/* Shop Selection */}
            <div className="mb-8">
              <label className="block text-sm font-semibold text-gray-700 mb-4 uppercase tracking-wide">🏪 Select Shop</label>
            ...
            {selectedShop && (
              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-4 uppercase tracking-wide">💰Select Discount</label>
                <div className="relative max-w-md">
                  <button
                  ...
                  >
                    <span className={selectedDiscount ? "text-gray-900 font-medium" : "text-gray-500"}>
                      {selectedDiscount ? ` (${selectedShop.discount_harvest}%)` : "Choose discount"}
                    </span>
                  ...
                  {isDiscountDropdownOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-gray-200 rounded-2xl shadow-xl z-10">
                      <button
                        onClick={() => {
                          setSelectedDiscount('discount_harvest');
                      ...
                      >
                        ...
                        <div className="text-sm text-gray-600 mt-1">{selectedShop.discount_harvest}% off</div>
                      </button>
                    </div>
                  )}
                </div>
            ...
            {selectedProducts.length === 0 ? (
              ...
              <div className="space-y-4">
                {selectedProducts.map((p, index) => (
                  <div key={p.id} className="flex items-center justify-between p-6 bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl border border-gray-200 hover:border-blue-300 transition-all duration-200 group">
                    ...
                    <div className="flex items-center space-x-6">
                      <div className="text-right">
                        <p className="font-bold text-gray-900 text-lg">${(p.tp_price * p.quantity).toFixed(2)}</p>
                        <p className="text-sm text-gray-600">${p.tp_price.toFixed(2)} × {p.quantity}</p>
                      </div>
                      <button onClick={() => removeProduct(p.id)} className="opacity-0 group-hover:opacity-100 p-3 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-xl transition-all duration-200">
