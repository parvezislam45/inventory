  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 p-6">
      <div className="max-w-7xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Kazi Orders Management</h1>
          <p className="text-gray-600">View and manage all Kazi brand orders</p>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
            <div className="max-w-md mx-auto">
              ...
              <p className="text-gray-600 mb-6">
                Please select a shop from the dropdown above to view their Kazi brand orders.
              </p>
            </div>
          </div>
        )}
        ...
        {!loading && selectedShop && (
          <div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-xl font-semibold text-gray-900">
                    {shops.find(s => s.id === selectedShop)?.shop_name} - Kazi Orders
                  </h2>
                  <p className="text-gray-600 mt-1">
                    {orders.filter(order => order.items?.some(item => item.brand === "Kazi")).length} invoices found
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm text-gray-600">Total Kazi Orders Value</p>
                  <p className="text-2xl font-bold text-gray-900">
                    ৳{orders
                      .reduce((total, order) => {
                        const kaziItems = order.items?.filter(item => item.brand === "Kazi") || [];
                        return total + kaziItems.reduce((sum, item) => sum + Number(item.final_price), 0);
                      }, 0)
            ...
            <div className="grid gap-6">
              {orders.map(order => {
                const kaziItems = order.items?.filter(item => item.brand === "Kazi") || [];
                if (kaziItems.length === 0) return null;
                const kaziTotal = kaziItems.reduce((sum, item) => sum + item.final_price, 0);
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="text-right ml-6">
                          <p className="text-2xl font-bold text-gray-900 mb-2">
                            ৳{Number(kaziTotal).toLocaleString(undefined, { minimumFractionDigits: 0, maximumFractionDigits: 2 })}
                          </p>

                          <button
                            onClick={() => router.push(`/kazi/invoices/${order.id}`)}
                      ...
                      <div className="border-t border-gray-200 pt-4">
                        <h4 className="font-semibold text-gray-900 mb-3 flex items-center">
                          ...
                          Kazi Products ({kaziItems.length})
                        </h4>
            ...
            {orders.filter(order => order.items?.some(item => item.brand === "Kazi")).length === 0 && (
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                <div className="max-w-md mx-auto">
                  ...
                  <h3 className="text-xl font-semibold text-gray-900 mb-2">No Kazi Orders Found</h3>
                  <p className="text-gray-600">
                    No Kazi brand orders found for {shops.find(s => s.id === selectedShop)?.shop_name}.
                  </p>
                </div>
              </div>
            )}
